...
lint: ## check style with flake8 and isort
	flake8 pygridsim tests
	isort -c --recursive pygridsim tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find pygridsim tests -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive pygridsim tests
	isort --apply --atomic --recursive pygridsim tests

.PHONY: coverage
coverage: ## check code coverage quickly with the default Python
	coverage run --source pygridsim -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --separate --no-toc -o docs/api/ pygridsim
	$(MAKE) -C docs html
